...
// Export for SharePoint - Clean tabular format that can be uploaded directly
// This replaces the file in SharePoint for others to view
export const exportForSharePoint = (state: {
  projects: Array<{
    id: string;
    code: string;
    clientName: string;
  ...
  // Projects sheet - clean tabular format
  const projectsData = [
    ['ProjectCode', 'ClientName', 'Address', 'Status', 'CPEnabled', 'CreatedDate', 'Notes'],
  ...
  const link = document.createElement('a');
  link.href = url;
  link.download = `HOC Internal Dash Sync [Dont Edit].xlsx`;
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
  URL.revokeObjectURL(url);
